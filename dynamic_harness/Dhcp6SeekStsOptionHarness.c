...
void verify_Dhcp6SeekStsOption()
{
     ...
     int    type;
     klee_make_symbolic(&type , sizeof(int), "Instance->Config->IaDescriptor.Type");
     Instance->Config->IaDescriptor.Type = type;
    
     UINT32    IaId;
     klee_make_symbolic(&IaId , sizeof(UINT32), "Instance->Config->IaDescriptor.IaId");
     Instance->Config->IaDescriptor.IaId = IaId;
