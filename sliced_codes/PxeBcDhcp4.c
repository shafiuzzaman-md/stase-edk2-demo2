  ...
  )
{
  ...
  {
    klee_assert(PxeOption >= 0 || PxeOption < (Offset < VendorOptionLen) && (PxeOption->OpCode != DHCP4_TAG_EOP)) ;
    PxeOption = (EFI_DHCP4_PACKET_OPTION *)(Dhcp4Option->Data + Offset);
    ...
    if(PxeOption->Length >= sizeof(UINT32)){
        UINT32 bitmapSize = sizeof(BitMap) / sizeof(BitMap[0]);  // Calculate the number of elements in BitMap
        UINT32 bitIndex = *(UINT32 *)(PxeOption->OpCode);
        UINT32 arrayIndex = bitIndex / 32;    // Calculate the corresponding array index
        klee_assert(arrayIndex >= 0 && arrayIndex < bitmapSize) ;
    } 
  ...
  if (!Iwc) {
    ...
    if(Dhcp4Option->Length >= sizeof(UINT32)){
        UINT32 bitmapSize = sizeof(BitMap) / sizeof(BitMap[0]);  // Calculate the number of elements in BitMap
        UINT32 bitIndex = *(UINT32 *)(Dhcp4Option->Data);
        UINT32 arrayIndex = bitIndex / 32;    // Calculate the corresponding array index
        klee_assert(arrayIndex >= 0 && arrayIndex < bitmapSize) ;
    } 
  ...
  )
{
  ...
  klee_make_symbolic(&IsPxeOffer, sizeof(IsPxeOffer),"IsPxeOffer");
  klee_make_symbolic(&Option, sizeof(Option),"Private->DhcpAck.Dhcp4->OptList");
  //klee_make_symbolic(&Cache4->VendorOpt, sizeof(Cache4->VendorOpt),"&Cache4->VendorOpt");
  ...
  )
{
  ...
  EFI_STATUS                Status;
  PXEBC_DHCP4_PACKET_CACHE  *Cache4 = malloc(sizeof(PXEBC_DHCP4_PACKET_CACHE));
  EFI_DHCP4_PACKET          *Reply;

  
  klee_make_symbolic(Cache4, sizeof(*Cache4), "*Cache4");
  ...
  )
{
  ...
  //(PxeBcRetryBinlOffer (Private, Index));

  klee_assume(Private->SelectIndex > 0);
  SelectIndex = (UINT32)(Private->SelectIndex - 1);
  printf("SelectIndex:%d\n",SelectIndex);
  //ASSERT (SelectIndex < PXEBC_OFFER_MAX_NUM);
  //Cache4  = &Private->OfferBuffer[SelectIndex].Dhcp4;
    EFI_DHCP4_PACKET_OPTION **OptList = malloc(PXEBC_DHCP4_TAG_INDEX_MAX * sizeof(EFI_DHCP4_PACKET_OPTION*));
    
    for (int i = 0; i < PXEBC_DHCP4_TAG_INDEX_MAX; i++) {
        OptList[i] = malloc(sizeof(EFI_DHCP4_PACKET_OPTION));
        klee_make_symbolic(OptList[i], sizeof(EFI_DHCP4_PACKET_OPTION), "Private->OfferBuffer->Dhcp4->OptList");
    }

  //EFI_DHCP4_PACKET_OPTION    *OptList[PXEBC_DHCP4_TAG_INDEX_MAX] = malloc(sizeof(EFI_DHCP4_PACKET_OPTION));
 // klee_make_symbolic(OptList, sizeof(EFI_DHCP4_PACKET_OPTION), "Private->OfferBuffer->Dhcp4->OptList");
  Options = Cache4->OptList;
  Status  = EFI_SUCCESS;
 // klee_assert(0);
